...
class Ball(Object2D):
    def __init__(self, radius, position, force, load_image=True):
        ...
        if load_image:
            self.image = pygame.image.load('../Graphics/'+str(radius)+'.png')
    ...
    def update(self, time_passed):
        ...
        if self.x < self.radius or self.x > SCREEN_WIDTH - self.radius:
            ...
            if self.x < self.radius:
                ...
                self.position = Vec2D(2*(SCREEN_WIDTH-self.radius) - self.x, self.y)
                
        if self.y > SCREEN_HEIGHT - self.radius:
            self.position = Vec2D(self.x, 2 * (SCREEN_HEIGHT - self.radius) - self.y)
            ...
            if self.y < self.max_height:
                ...
                if self.falling:
                    self.force = Vec2D(self.force.x,
                                       -(abs((self.y - self.max_height) * 2 * GRAVITY)**.5))
                else:
                    self.force = Vec2D(self.force.x, -self.force.y)
